...
# l2 norm of difference between tensors
def norm_diff(v1, v2):
...
# estimator of smoothness according to paper
def eval_smoothness(file, new_model, data, labels, loss_object, num_pts=1):
    old_model = load_model(file)
    # evaluating only on x percent of the data
    # as suggested in the paper 10% are representative
    percentage = 10
    ...
    gnorm = norm(old_grad)
    update_size = norm_diff(new_model.trainable_weights, old_model.trainable_weights)
    max_smooth = tf.constant(-1.0, dtype=tf.float32)
    for alpha in alphas:
        # copy of old_model
        between_model = load_model(file)
        # overwrite weights to obtain x_old + alpha*d
        ...
        smooth = norm_diff(old_grad, between_grad) / (update_size * (1 - alpha))
        max_smooth = tf.math.maximum(smooth, max_smooth)
    # convert from Tensor to float for further visualization
    max_smooth = max_smooth.numpy()
    gnorm = gnorm.numpy()
    # avoid clutter from old models and layers, especially when memory is limited
...
def tune_hyperparameters_and_save_losses_to_file(build_model_function, data, labels, learning_rates, clip_norms,
                                                 n_epochs, batch_size, dir_name):
